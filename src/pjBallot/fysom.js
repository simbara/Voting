...
;
	...
	, 1, [null,null,['self'],['cfg']]);
		...
		$method = $pyjs__bind_method2('isstate', function(state) {
			...
			var $attr1,$attr2;
			return $p['op_eq'](self['current'], state);
		}
	, 1, [null,null,['self'],['state']]);
		...
		$method = $pyjs__bind_method2('can', function(event) {
			...
			return ($p['bool']($and1=self['_map'].__contains__(event))?($p['bool']($and2=self['_map'].__getitem__(event).__contains__(self['current']))?!$p['bool']($p['hasattr'](self, 'transition')):$and2):$and1);
		}
					...
					kwargs = $p['__empty_dict']();
{
				...
				var $attr9,src,$iter5_idx,e,k,_tran,$iter5_array,dst,$iter5_type,$attr15,$iter5_nextval,$iter5_iter,$attr14,$attr16,$attr11,$attr10,$attr13,$attr12;
				...
				if ($p['bool'](!$p['bool'](self['can'](event)))) {
					throw ($m['FysomError']($p['sprintf']('event %s inappropriate in current state %s', $p['tuple']([event, self['current']]))));
				}
				src = self['current'];
				dst = self['_map'].__getitem__(event).__getitem__(src);
				$m['_e_obj'] = (function(){
					var $cls_definition = new Object();
					var $method;
					$cls_definition.__module__ = 'fysom';
				...
				if ($p['bool'](!$p['op_eq'](self['current'], dst))) {
	...
	, 1, [null,null,['self'],['event']]);
		...
		$method = $pyjs__bind_method2('_before_event', function(e) {
			...
			fnname = $p['__op_add']($add1='onbefore',$add2=e['event']);
	...
	, 1, [null,null,['self'],['e']]);
		...
		$method = $pyjs__bind_method2('_after_event', function(e) {
			...
			var fnname,$iter6_idx,$attr21,$iter6_type,$attr22,$attr20,$add3,$iter6_array,$attr19,$add4,$add5,$add6,$iter6_iter,$iter6_nextval;
			$iter6_iter = $p['list']([$p['__op_add']($add3='onafter',$add4=e['event']), $p['__op_add']($add5='on',$add6=e['event'])]);
	...
	, 1, [null,null,['self'],['e']]);
		...
		$method = $pyjs__bind_method2('_leave_state', function(e) {
			...
			fnname = $p['__op_add']($add7='onleave',$add8=e['src']);
			if ($p['bool']($p['hasattr'](self, fnname))) {
				return $p['getattr'](self, fnname)(e);
			}
			return null;
	...
	, 1, [null,null,['self'],['e']]);
		...
		$method = $pyjs__bind_method2('_enter_state', function(e) {
			...
			var $attr27,fnname,$add12,$iter7_nextval,$add9,$iter7_iter,$iter7_array,$add10,$attr25,$attr26,$iter7_idx,$attr28,$iter7_type,$add11;
			$iter7_iter = $p['list']([$p['__op_add']($add9='onenter',$add10=e['dst']), $p['__op_add']($add11='on',$add12=e['dst'])]);
